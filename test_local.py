...
if __name__ == '__main__':
    ...
    x_mean = lafan_data_test.x_mean_local.to(device)
    x_std = lafan_data_test.x_std_local.to(device)
    x_mean_n = lafan_data_test.x_mean_local.view(1, 1, opt['model']['num_joints'], 3).to(device)
    x_std_n = lafan_data_test.x_std_local.view(1, 1, opt['model']['num_joints'], 3).to(device)
    ...
    for n_trans in trans_lengths:
        ...
        for batch_i, batch_data in enumerate(lafan_loader_test):
            ...
            with torch.no_grad():
                ...
                interp_local_p = interp_pose[:, :, 0:opt['model']['num_joints'] * 3].reshape(interp_pose.shape[0],interp_pose.shape[1], -1, 3)
                interp_local_q = interp_pose[:, :, opt['model']['num_joints'] * 3:].reshape(interp_pose.shape[0],interp_pose.shape[1], -1, 4)

                trans_interp_local_quats = interp_local_q[:, opt['model']['n_past']:opt['model']['n_past']+n_trans, :, :]
                trans_interp_local_poses = interp_local_p[:, opt['model']['n_past']:opt['model']['n_past']+n_trans, :, :]
                # # # Normalize
                trans_interp_local_poses = (torch.from_numpy(trans_interp_local_poses) - x_mean_n.cpu()) / x_std_n.cpu()

                # prediction and gt data processing------------------
                # rotations
                local_q_gt = target_output[:, :, opt['model']['num_joints'] * 3:]  # B, F, J*4
                local_q_gt_ = local_q_gt.view(local_q_gt.size(0), local_q_gt.size(1), -1, 4)  # ground truth rotation and position data
                local_q_pred = output[:, :, opt['model']['num_joints'] * 3:]  # B, F, J*4            局部四元数
                local_q_pred_ = local_q_pred.view(local_q_pred.shape[0], local_q_pred.shape[1], -1, 4)  # pred r and p data

                # positions
                local_p_pred = output[:, :, 0:opt['model']['num_joints'] * 3]  # B, F, J*3
                local_p_pred = local_p_pred.view(local_p_pred.shape[0], local_p_pred.shape[1], -1, 3)
                local_p_pred_ = local_p_pred / torch.norm(local_p_pred, dim=-1, keepdim=True)
                local_p_gt = target_output[:, :, 0:opt['model']['num_joints'] * 3]  # B, F, J*3
                local_p_gt = local_p_gt.view(local_p_gt.size(0), local_p_gt.size(1), -1, 3)  # B, F, J, 3
                # local_p_gt_ = local_p_gt / torch.norm(local_p_gt, dim=-1, keepdim=True)
                ...
                trans_local_p_pred = (local_p_pred_[:, opt['model']['n_past']: opt['model']['n_past']+n_trans,
                                     ...] - x_mean_n) / x_std_n  # Normalization
                trans_local_q_pred = local_q_pred_[:, opt['model']['n_past']:opt['model']['n_past']+n_trans,
                                    ...]  # 过渡区间的局部旋转真实值  B, n_trans, J, 4

                trans_local_p_gt = (local_p_gt[:, opt['model']['n_past']: opt['model']['n_past']+n_trans,
                                    ...] - x_mean_n) / x_std_n


                trans_local_q_gt = local_q_gt_[:, opt['model']['n_past']: opt['model']['n_past']+n_trans, ...]

                # zero-v data processing-----------------------------
                zerov_trans_local_quats, zerov_trans_local_poses = np.zeros_like(trans_local_q_gt.detach().cpu().numpy()), np.zeros_like(
                    trans_local_p_gt.detach().cpu().numpy())
                zerov_trans_local_quats[:, :, :, :] = local_q_gt_[:, opt['model']['n_past'] - 1:opt['model']['n_past'], :, :].detach().cpu().numpy()
                trans_zerov_local_quats = zerov_trans_local_quats
                zerov_trans_local_poses[:, :, :, :] = local_p_gt[:,opt['model']['n_past'] - 1:opt['model']['n_past'], :, :].detach().cpu().numpy()
                # # Normalize
                trans_zerov_local_poses = (torch.from_numpy(zerov_trans_local_poses) - x_mean_n.cpu()) / x_std_n.cpu()
                ...
                # local to global---
                trans_global_q_gt, trans_global_p_gt = uf.quat_fk(trans_local_q_gt.detach().cpu().numpy(),trans_local_p_gt.detach().cpu().numpy(), parents)
                trans_zerov_global_quats, trans_zerov_global_poses = uf.quat_fk(trans_zerov_local_quats, trans_zerov_local_poses, parents)
                trans_interp_global_quats, trans_interp_global_poses = uf.quat_fk(trans_interp_local_quats, trans_interp_local_poses, parents)
                trans_pred_global_quats, trans_pred_global_poses = uf.quat_fk(trans_local_q_pred.detach().cpu().numpy(), trans_local_p_pred.detach().cpu().numpy(), parents)


                l2q_error_zerov = np.mean(
                    np.sqrt(np.sum((trans_zerov_global_quats - trans_global_q_gt) ** 2.0,axis=(2, 3))))
                l2q_error_interp = np.mean(
                    np.sqrt(np.sum((trans_interp_global_quats - trans_global_q_gt) ** 2.0, axis=(2, 3))))
                l2q_error_our = np.mean(
                    np.sqrt(np.sum((trans_pred_global_quats - trans_global_q_gt) ** 2.0,axis=(2, 3))))
                # 插值的全局旋转四元数 - 真实的全局旋转四元数 二范数 最后求均值
                # Global positions loss L2P
                l2p_error_zerov = np.mean(np.sqrt(
                    np.sum((trans_zerov_global_poses - trans_global_p_gt) ** 2.0,axis=(2, 3))))
                l2p_error_interp = np.mean(np.sqrt(
                    np.sum((trans_interp_global_poses - trans_global_p_gt) ** 2.0,axis=(2, 3))))
                l2p_error_our = np.mean(np.sqrt(
                    np.sum((trans_pred_global_poses - trans_global_p_gt) ** 2.0, axis=(2,3))))

                # NPSS loss on global quaternions
                npss_error_zerov = ben.fast_npss(ben.flatjoints(trans_global_q_gt),
                                               ben.flatjoints(trans_zerov_global_quats))

                npss_error_interp = ben.fast_npss(ben.flatjoints(trans_global_q_gt),
                                               ben.flatjoints(trans_interp_global_quats))

                npss_error_our = ben.fast_npss(ben.flatjoints(trans_global_q_gt),
                                             ben.flatjoints(trans_pred_global_quats))
                ...
                if opt['test']['save_bvh']:
                    ...
                    # local_q_pred, local_p_pred = uf.quat_ik(local_q_pred_.detach().cpu().numpy(), local_p_pred_.detach().cpu().numpy(), parents)
                    # local_q_gt, local_p_gt = uf.quat_ik(local_q_gt_.detach().cpu().numpy(), local_p_gt.detach().cpu().numpy(), parents)
                    local_q_pred = local_q_pred_.reshape((local_q_pred_.shape[0],local_q_pred_.shape[1],-1))
                    local_p_pred = local_p_pred_.reshape((local_p_pred_.shape[0], local_p_pred_.shape[1], -1))
                    local_p_gt = local_p_gt.reshape((local_p_gt.shape[0], local_p_gt.shape[1], -1))
                    local_q_gt = local_q_gt.reshape((local_q_gt.shape[0], local_q_gt.shape[1], -1))
                ...
                if opt['test']['save_img'] and opt['test']['save_gif'] and batch_i < 50:
                    ...
                    position_0 = local_p_gt[0, 0, ...].detach().to('cpu').numpy()
                    position_1 = local_p_gt[0, -1, ...].detach().to('cpu').numpy()
                    for t in range(opt['model']['seq_length']):
                        # print(type(position_0))
                        # print(position_0.device)
                        plot_pose(
                            np.concatenate([position_0, local_p_pred_[0, t].detach().to('cpu').numpy(), position_1], axis=0),t,
                            img_dir + '/pred_batch_' + str(batch_i))
                        plot_pose(np.concatenate([position_0,  # .detach().cpu().numpy(), \
                                                  local_p_gt[0, t].detach().to('cpu').numpy(),
                                                  position_1], 0), \
                                  t,
